...
> keeps the original openclaw instance bit-identical across `tofu apply`.
> If you're standing this role up **for the first time** (e.g. DR rebuild,
> new lab), drop those two pin lines from
> `vms/openclaw/terraform/terraform.tfvars.tpl` BEFORE running `just hydrate`
...
`just apply openclaw` stays a no-op for storage.

Two paths to move the disk to nas-vms:

**A. In-place move via Proxmox (preserves the VM, no recreate).**
Recommended — keeps channel pairings + `~/.openclaw/` state intact
without re-onboarding.

```bash
ssh root@pve12t 'qm move-disk 8032 scsi0 nas-vms --delete 1'
ssh root@pve12t 'qm move-disk 8032 ide2 nas-vms --delete 1'   # cloud-init drive
```

(Substitute the actual `proxmox_node` and VMID if they differ from the
defaults.) Then remove the pin from `vms/openclaw/terraform/terraform.tfvars.tpl`,
re-run `just hydrate openclaw`, and `just plan openclaw`. Plan should
...
lines, re-hydrate, `just apply openclaw` — the boot disk is destroyed
and recreated on nas-vms. Re-install openclaw, re-onboard channels
(or restore from a `openclaw backup` archive per **Backup + restore**
above).

...
> keeps the original nemoclaw instance bit-identical across `tofu apply`.
> If you're standing this role up **for the first time** (e.g. DR rebuild,
> new lab), drop those two pin lines from
> `vms/nemoclaw/terraform/terraform.tfvars.tpl` BEFORE running `just hydrate`
> so the fresh VM lands on `nas-vms` (cluster-shared NFS, the role default
> for cluster-mobility). If you're operating an existing instance, leave
> the pin alone — see [Storage migration](#storage-migration-local-lvm--nas-vms)
...
`just apply nemoclaw` stays a no-op for storage.

Two paths to move the disk to nas-vms:

**A. In-place move via Proxmox (preserves the VM, no recreate).**
Recommended — keeps the sandbox image cache + OpenClaw state +
channel pairings intact without re-onboarding.

```bash
ssh root@pve13m 'qm move-disk 8033 scsi0 nas-vms --delete 1'
ssh root@pve13m 'qm move-disk 8033 ide2 nas-vms --delete 1'   # cloud-init drive
```

(Substitute the actual `proxmox_node` and VMID if they differ from the
defaults.) Then remove the pin from `vms/nemoclaw/terraform/terraform.tfvars.tpl`,
re-run `just hydrate nemoclaw`, and `just plan nemoclaw`. Plan should
report no changes (or only `datastore_id` attribute reconciliation) —
if it wants to destroy and recreate the disk, abort and verify the
move first.

**B. Destructive recreate via tofu (loses state).** Drop the pin
lines, re-hydrate, `just apply nemoclaw` — the boot disk is destroyed
and recreated on nas-vms. Re-install nemoclaw, re-onboard model
providers and channels (or restore from a `nemoclaw snapshot create`
archive per **Backup + restore** above).

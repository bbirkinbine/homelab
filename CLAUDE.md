...
## Active context (as of 2026-06-01)
...
**4-node Proxmox cluster is live.** The lab runs a corosync cluster (`homelab`), quorate since 2026-05-13 and expanded from 3 to 4 nodes on 2026-06-01: nodes `pve12t` / `pve13m` / `pve13t` / `pve12t2`, corosync ring0 on the LAN and ring1 on the Thunderbolt fabric, live migration over the TB fabric, cluster firewall enabled, and NFS-shared storage (`nas-vms`) from the Asustor AS6706T registered cluster-wide. The TB fabric is a line — `pve12t — pve13m — pve13t — pve12t2` — with both `pve13m` and `pve13t` acting as L3 transit nodes (`pve13t` was promoted from leaf to transit when `pve12t2` joined onto its formerly-spare TB port; `pve12t`↔`pve12t2` is a 3-hop path). Authoritative design lives in the project's private design vault — ask the maintainer if you need access. Working guidance for the cluster:

- **Proxmox hostnames stay `pveXX` (`pve12t`, `pve13m`, `pve13t`, `pve12t2`).** Physical-chassis labels in the design vault (`nuc12 / nuc13-mini / nuc13-tall`) are NOT Proxmox node names — don't rename one to the other.
...
- **NFS shared storage (`nas-vms`) is the default for cluster-mobile roles.** Registered cluster-wide per ADR-0004; openbao / openclaw / nemoclaw default both `disk_storage` and `snippets_storage` to `nas-vms` via their per-role variables. Existing instances of those three carry a `disk_storage = "local-lvm"` + `snippets_storage = "local"` pre-flip pin in their committed `.tfvars.tpl` so `tofu apply` stays a no-op for storage until the pin is dropped (destructive: removing it recreates the disk; use `qm move-disk` for an in-place move). `amp-game` and `rootca` override the cluster-mobile default with `local-lvm` — game-server I/O latency for amp-game (explicit call during the 2026-05-14 port), HSM USB passthrough pinning rootca to pve12t — and carry the reason in their variable descriptions.
...
**OpenTofu + Ansible migration COMPLETE (2026-05-19).** Every VM role under `vms/` uses the canonical shape: OpenTofu provisioning (per-role `terraform/`), Ansible config (per-role `ansible/roles/<role>/`), identity-only cloud-init (per-role `cloud-init/user-data.yaml.tftpl`). Shared module: `modules/proxmox-vm/`. New roles copy `vms/_template/` (generic cluster-mobile skeleton, placeholder `__ROLE__` for sed-rename, valid HCL/YAML throughout — passes `tofu fmt -check` + `tofu validate` + ansible syntax-check). Hardware-pinned variants cross-reference `vms/rootca/` (USB passthrough + air-gap toggle) or `vms/llm/` (eGPU passthrough via cluster-wide PCI mapping). Legacy `deploy.sh` artifacts preserved only where the operator-discovery context still matters: `vms/openbao/legacy/` (USB-passthrough port discovery) and `vms/amp-game/legacy/` (cloud-init drive-recreate dance + the apt-daily/unattended-upgrades pattern that moved to Ansible).

**Cross-cutting tooling:** `Justfile` at repo root (recipes parameterized by role name: `hydrate`, `check`, `plan`, `apply`, `ansible`, `inventory`, `destroy`, `fmt`, `validate`, `check-roles`, `shell-lint`). `scripts/preflight.sh` — per-role ssh/Proxmox/template/snippets checks before `tofu apply` (network-touching). `scripts/hydrate.sh` — resolves `kp://...` placeholders in `terraform.tfvars.tpl` against KeePassXC. `scripts/check-role-consistency.sh` — repo-wide static analysis across `vms/*/terraform/` (six checks: fmt, storage knob surface, storage knob wiring, dead-code references, canonical file set, VMID conformance per ADR-0008). Run `just check-roles` before any PR that touches a role definition or the shared module. Skips `vms/_*` dirs (template / scaffolding convention).

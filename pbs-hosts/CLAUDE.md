...
13. **`config_backup.yml` — PBS config self-backup (opt-in, default OFF).** Gated on `pbs_host_manage_config_backup` (default false) via the `import_tasks` `when:`, placed **last** in `main.yml` (it writes to the NAS mount, so `nfs.yml` must run first). PBS ships nothing to back up its own `/etc/proxmox-backup`; this deploys the shared `scripts/pbs-config-backup.sh` + a systemd timer→oneshot that tars `/etc/proxmox-backup` to a timestamped `.tar.gz` under `pbs_config_backup_dest_dir` on the NAS (a sibling of the datastore, not inside it), then ages out tarballs older than `pbs_config_backup_keep_days`. Once applied, the host self-backs-up on schedule with no operator action. **Deliberately a plain tarball, NOT `proxmox-backup-client` into a datastore:** a datastore is a chunk store readable only by a *running* PBS, so a config backup living inside it can't be opened until PBS is reinstalled and the datastore re-attached — backwards for a DR artifact (the thing that helps you rebuild is locked behind the rebuild). A tarball is openable from anywhere with no PBS in the loop, and the NAS replicates it to the secondary. An earlier API-based design (token + namespace + ACL + `proxmox-backup-client`) was built then dropped for exactly this reason — see SCAFFOLD-NOTES. The backup's DR value is `datastore.cfg` (re-attach the chunk store) + `authkey.key` + the user/token shadows — restoring them keeps already-issued tokens (incl. the cluster's `pveingress`) valid across a rebuild. **No API, no token, no secret on the host** — the job runs as root via the timer and reads the dir directly. The unit's `RequiresMountsFor` + a `findmnt`/`mountpoint` guard in the script refuse to write to local disk if the NAS mount is down. The tarball carries `authkey.key`, so it's written `0600` into a `0700` dir; the NAS export is LAN-only + ACL'd. **Applying the role must never trigger an out-of-schedule backup** — the timer owns scheduling; reload/restart tasks are `when: <unit> is changed`. Mirror of the `ups.yml` deploy pattern. See `pbs-hosts/README.md` "PBS config self-backup" for the operator runbook + restore procedure.

...
11. **`ups.yml` — UPS shutdown guardian (opt-in, default OFF).** Gated on `pve_host_manage_ups_guardian` (default false) via the `import_tasks` `when:`. Installs `nut-client`, deploys the shared `scripts/nas-ups-guardian.sh` to `/usr/local/sbin/nas-ups-guardian`, renders `/etc/default/nas-ups-guardian` (`GUARDIAN_MODE=pve` + NUT target + thresholds), and installs + enables a systemd timer→oneshot that polls the UPS with `upsc` every ~20s. On battery AND `battery.charge ≤ pve_host_ups_charge_threshold` (default 50), it `qm shutdown`s every guest on the node, force-stops stragglers, then powers off — leaving the NAS (10% cutoff) to go last. Ordering is a charge gap, NOT the NUT primary/secondary handshake (the Asustor upsd does not wait for network clients). **Applying the role must never power off a host** — the guardian acts only on a real power event; on line power every poll is a no-op. Mirror of the `pbs-host` role's `ups.yml` (pbs01 leads at 60%). See `pve-hosts/README.md` and the vault doc `[[nut-ordered-shutdown-design]]`. Reload/restart tasks are `when: <unit> is changed` to keep a healthy re-run at `changed=0`.

...
## 2026-06-01 — 4-node TB line extension (pve12t2)

A second NUC12 Pro Tall (`pve12t2`) joined the cluster, cabled into pve13t's formerly-spare TB port. That extended the line to `pve12t — pve13m — pve13t — pve12t2` and **promoted pve13t from leaf to transit** (both pve13m and pve13t now forward; `pve12t`↔`pve12t2` is a 3-hop, two-forward path). Addressing follows the existing scheme: loopback `10.10.10.15`, link `/31` `10.10.0.4/31` (pve13t `.4` ↔ pve12t2 `.5`). Vault address plan extended in `[[Thunderbolt Mesh Networking — 3-Node Cluster Option]]` § "4-node extension".

- **`interfaces.j2` routing reworked from a `tb_role` branch to a per-link `reaches` list.** The old template branched on leaf vs transit: a leaf routed every peer via its single neighbor; a transit node routed only its *directly-connected* peers. That transit assumption is only correct for a 3-node line — at four nodes each middle node has a peer two hops away (pve13m→pve12t2, pve13t→pve12t) that the old branch never routed, silently dropping the far node. The template now iterates `tb_links[].reaches` (the nodes on the far side of each link) and emits one `via <peer-/31> dev <link> src <loopback>` route per entry — correct for a line of any length, and it removes the role distinction from the route plan entirely. Rendered routes for the original three nodes are unchanged except for the additive pve12t2 route (verified by offline render before apply). `reaches` is now a required field on each `tb_links` entry — omit it and the node gets no TB routes. Shipped on branch `feat/add-pve12t2-node`.

- **GOTCHA — after a host reboot, the TB `tbnet-*` interfaces come up admin-DOWN and stay down until a post-boot `ifreload -a`.** This is the boot-ordering race the `.link` pinning was meant to survive, but pinning only fixes the *name* — `networking.service` still runs `ifup` before the renamed netdev exists, so the `auto tbnet-*` stanzas miss it and the interface never gets its address. The role deliberately never auto-reloads (`network.yml` header explains why), so the operator MUST run `ifreload -a` after every reboot. During the pve12t2 bring-up this had silently taken the **entire TB fabric + corosync ring1 down fleet-wide** after an earlier reboot — every node's `tbnet-*` was admin-down, the cluster had been running on ring0/LAN only, unnoticed. Recovery was `ifreload -a` on every node in **two passes**: pass 1 brings all interfaces admin-up so the TB-net carriers negotiate (a link only lights up once *both* ends are up, so pass-1 route installs fail with `Nexthop has invalid gateway`); pass 2 re-runs the `post-up` routes now that carriers are present. If a future PVE upgrade keeps leaving the fabric down after reboots, the fix to consider is a `systemd` oneshot that runs `ifreload -a` after `network-online.target` + the thunderbolt modules — but that re-introduces the auto-reload-on-a-clustered-node hazard the role avoids, so it stays manual for now.

...
| `Homelab/Prometheus/` | `grafana-admin` | Password | Grafana `admin` user's UI password | operator UI login at `http://monitoring:3000` | [vms/monitoring/README.md](../vms/monitoring/README.md). Rotate with `sudo -u grafana grafana cli --homepath=/usr/share/grafana --config=/etc/grafana/grafana.ini admin reset-admin-password "$NEWPASS"` (run from a dir readable by the `grafana` user, e.g. `cd /usr/share/grafana` first). |

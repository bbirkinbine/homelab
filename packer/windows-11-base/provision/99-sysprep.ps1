...
# disk is the template artifact. Boot of the cloned VM goes through
# OOBE-mini and lands at cloudbase-init for per-clone identity.
#
...
# password (variables.pkr.hcl/var.build_password) so Packer can connect via
# WinRM. Sysprep /generalize clears machine SID and drivers — it does NOT
# clear local-account passwords or the AutoAdminLogon registry values
# written by the answer file. Without explicit cleanup, every clone would
# inherit a known-password admin account discoverable from the public
# Autounattend.xml. We solve this the same way the Ubuntu base does:
# defer the credential teardown to a one-shot scheduled task that fires
# AtStartup on the first boot of every clone, then self-destructs. Sysprep
# preserves scheduled-task definitions across /generalize, so the task
# rides into every clone unchanged. Templates never power on, so the task
# never fires on the template itself.
...
# Microsoft-blessed location for OEM/setup scripts that survives
# sysprep). The script is wrapped in a verbatim here-string so $vars
# inside are LITERAL at install time — they evaluate when the cleanup
# script itself runs on the clone, not now.
...
# Windows 11 base template. Installed by provision/99-sysprep.ps1 and
# scheduled to fire AtStartup as SYSTEM. One-shot: self-destructs at the
# end so it cannot fire twice.
#
...
try {
    ...
    # 1. Wait for cloudbase-init to reach Stopped (it auto-stops after its
    #    Plugins phase). Bound by ~120s — a stuck service must not block
    #    the credential lockdown forever.
    $cbi = Get-Service -Name 'cloudbase-init' -ErrorAction SilentlyContinue
    if ($cbi) {
...
# a 5-minute hard execution limit so a stuck wait loop can't park the task
# forever. -Force replaces any prior registration of the same name.
